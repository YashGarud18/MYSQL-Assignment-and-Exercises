...
-- 3. Procedure to get bookings by customer ID
DELIMITER //
CREATE PROCEDURE GetBookingsByCustomer(IN p_CustomerID INT)
BEGIN
    SELECT * FROM Bookings WHERE CustomerID = p_CustomerID;
END //
DELIMITER ;

-- Call the procedure:
CALL GetBookingsByCustomer(1);


-- 4. Procedure to update booking status
DELIMITER //
