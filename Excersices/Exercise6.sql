...
SELECT Flight_Number, SUBSTRING(Flight_Number, 3, 2) AS Flight_Substring FROM Flights; 
...
SELECT Flight_Number, LEFT(Flight_Number, 2) AS First_Character FROM Flights;

-- 14. Get the Last Character of Each Arrival Airport Name
SELECT Arrival_Airport, RIGHT(Arrival_Airport, 5) AS Last_Character FROM Flights;
...
SELECT Flight_Number, LOCATE('10', Flight_Number) AS Position_of_AI FROM Flights;
...
SELECT COUNT(*) AS Flights_With_Over_50_Seats FROM Flights WHERE Seats_Available > 60;
...
FROM Flights WHERE seats_available > 50;
